...
export function CashFlow() {
  ...
  const history = useMemo(() => [...forexCashFlows].reverse(), [forexCashFlows]);
  ...
  const columns: Column<CashFlowRow>[] = [
    ...
    {
      key: 'tipe', header: 'Type', width: 'minmax(130px,1fr)',
      cell: cf => <span className={isInflow(cf.tipe) ? 'text-adm-up' : 'text-adm-down'}>{cf.tipe}</span>,
    },
    { key: 'account_type', header: 'Account', width: '100px', cell: cf => <StatusBadge kind="neutral" label={cf.account_type.toUpperCase()} /> },
    {
      key: 'jumlah', header: 'Amount', numeric: true, width: '120px', sortValue: cf => cf.jumlah,
      // Signed by direction: inflows +, outflows − (color never the sole carrier).
      cell: cf => <span className={isInflow(cf.tipe) ? 'text-adm-up' : 'text-adm-down'}>{fmtSignedUsd(isInflow(cf.tipe) ? cf.jumlah : -cf.jumlah)}</span>,
    },
    {
      key: 'details', header: 'Details', width: 'minmax(110px,1fr)',
      cell: cf => (
        <span className="font-adm-data text-adm-micro text-adm-ink-dim">
          {cf.tipe.includes('Transfer') && cf.desk_tujuan ? `${cf.desk} → ${cf.desk_tujuan}` : cf.desk}
        </span>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-4">
      <PageHeader desk="forex" title="Cash Flow" sub="deposits · withdrawals · transfers" />

      <MetricStrip
        items={[
          { label: 'Funding account', value: forexBalances.funding, format: 'usd', sub: 'external deposits' },
          { label: 'Trading account', value: forexBalances.trading, format: 'usd', sub: 'available to trade' },
        ]}
      />

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {/* Form column — logic unchanged, adm chrome */}
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-adm border border-adm-line bg-adm-bg1 p-4 lg:col-span-1">
          ...
          <button
            ...
            className="w-full rounded-adm-sm border border-adm-line2 bg-adm-bg2 px-4 py-2 font-adm-data text-adm-xs uppercase text-adm-ink-hi hover:border-adm-up disabled:opacity-40"
          >
        ...
        <div className="lg:col-span-2">
          <p className="mb-2 font-adm-data text-adm-micro uppercase text-adm-ink-dim">Recent transactions — Forex</p>
          <DataTable columns={columns} rows={history} rowKey={cf => cf.id} empty="No Forex cash flow transactions yet." />
        </div>
      </div>
    </div>
  );
